...
部分情况下需要双重递归才能解决问题，例如力扣[437题](https://leetcode.cn/problems/path-sum-iii/?envType=study-plan-v2&envId=top-100-liked)

有些情况下双重递归会超时，但可以作为一种方法尝试，本题代码如下：
```java
    int result=0;
    ...
    public void backTrack(TreeNode node,long target)
    {
        if(node==null)
        {
            return;
    ...
    public void backTrackPath(TreeNode node, long cur, long target)
    {
        if(cur==target)
        {
            result++;
...
例如力扣[22题](https://leetcode.cn/problems/generate-parentheses/)

![image-20230908175044417](https://s2.loli.net/2023/09/08/7Y8MJsVgkWRUmi9.png)

这题需要注意一个规律，**左括号的数目必须大于等于右括号的数目**，这样才能继续进行下去
...
1. 当前操作：求第i个组成
2. 子问题？获得第i+1的部分
3. 下一个子问题？生成第大于i+1的部分

例如，力扣[131题](https://leetcode.cn/problems/palindrome-partitioning)

![image-20230909171132189](https://s2.loli.net/2023/09/09/frqD3k7syeoSCiY.png)

主要就是使用回溯法的思想，进行一步一步的分割，例如：

- 输入`aab`，可以将其分割为`a,ab`，`aa,b`两种情况
  1. 选择了`a`，对`ab`进行同样操作，进行下一步分割，依此类推
  2. 又或者选择了`aa`，则对`b`进行同样操作，依此类推
- 用一个数字来记录当前List中字符串的总长度，判断是否满足题设条件
- 类似回溯法解决子集问题，每次都判断有没有满足题设的解，有则加入到最终答案中
...
力扣[207题](https://leetcode.cn/problems/course-schedule/)

![image-20230807092719927](https://s2.loli.net/2023/08/07/z4IiHW2pMtkLsq8.png)

- 深度优先遍历

  可以通过深度优先来遍历图，遍历的过程中看是否重复访问过当前元素，重复访问则存在环
  ...
  ```java
      ...
      public void backTrack(HashMap<Integer,List<Integer>> courses,int cur)
      {
          ...
          //当前节点搜索已完成，不存在环，如果不设置为2，回溯回去后，这个节点值为1，会导致判断出现了环，从而出现错误解
          visited[cur]=2;
      }
  ```
...
力扣[886题](https://leetcode.cn/problems/possible-bipartition/)

![image-20230809143101717](https://s2.loli.net/2023/08/09/abFij3K1ItOAke5.png)

对于二分问题，一般的解决方案就是染色，比如说当前节点颜色为1，就将其相邻节点染色为2，遇到访问过的有颜色的节点，就比较节点颜色是否相同
